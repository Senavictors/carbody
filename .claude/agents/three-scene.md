...
- Pins HTML (`PINS`, `car-scene.css`) são projetados a cada frame via `projected.project(camera)` — não são elementos Three.js, são `<button>` posicionados via CSS custom properties. Cada sistema mostra no máximo 3 pins por vez (`shownPins`, `PINS.filter(p => p.system === activeSystem).slice(0, 3)`) — um novo pin só aparece de fato se o sistema tiver menos de 3 cadastrados; adicionar um 4º a um sistema já cheio não quebra nada, só não é exibido por padrão (a peça continua clicável direto na malha).
- **Animação condicionada ao sistema (desde `ADR-005`/`TASK-011`)**: o loop `render()` anima só as peças do sistema selecionado (ventoinha em `cooling`, rodas em `suspension`, polias da correia em `engine`), lendo `props.current.activeSystem`. O delta vem de `performance.now()` em segundos, limitado a 80 ms. Conjuntos que giram juntos são reparentados para um `THREE.Group` depois de construídos, via `attach()` (preserva a transformação no mundo); o slice de `car.children` precisa ser tirado antes do primeiro `attach()`, que muta o array. Os meshes continuam em `pickables`/`appearances`, então o raycasting não é afetado.
- A cena checa `prefers-reduced-motion` no `useEffect` principal (variável local + listener `change` removido no cleanup) — com a preferência ativa, nenhuma animação avança.
...
- **Pipeline de render (desde `ADR-002`/`TASK-001`)**: não é mais `renderer.render(scene, camera)` direto. É um `EffectComposer` com três passes, nesta ordem: `RenderPass` → `GTAOPass` (ambient occlusion leve, `blendIntensity` calibrado para sombra de contato sutil, não escurecimento forte) → `OutputPass` (aplica o tone mapping/color space do `renderer` — precisa ser sempre o último pass). `scene.environment` é um mapa gerado localmente via `THREE.PMREMGenerator` + `RoomEnvironment` (`three/addons/environments/RoomEnvironment.js`, sem HDR externo/rede) — dá reflexo a metais/vidros/borrachas. `resize()` chama `composer.setSize()` além de `renderer.setSize()`; o cleanup descarta `envRenderTarget`, `gtaoPass`, `outputPass` e `composer` (o `composer.dispose()` sozinho **não** descarta os passes adicionados — cada um precisa do próprio `.dispose()`, exceto `RenderPass`, que não aloca recurso GPU extra).
- **Peças decorativas vs. peças do catálogo (desde `TASK-002`/`ADR-003`)**: nem todo mesh precisa ser uma peça selecionável. Um mesh com `system` preenchido e `part` vazio (`''`) entra em `appearances` (responde à opacidade por sistema) mas não em `pickables` (não é clicável nem tem painel de detalhe) — é o padrão para detalhe visual aditivo sem entrada correspondente em `src/data/parts.ts` (ex.: os parafusos de roda e a mangueira decorativa do motor). Toda peça com `part` preenchido precisa ter uma entrada correspondente em `parts.ts` (dono: papel `content-catalog`) — os dois lados (geometria e conteúdo) usam o mesmo id combinado por convenção, não por tipo compartilhado.
...
6. **Toda animação nova respeita `prefers-reduced-motion` e é condicionada a um sistema** (`ADR-005`) — a cena não tem movimento ambiente, e uma animação que o usuário não pediu nem consegue desligar seria regressão de acessibilidade.
7. **Uma peça que se move precisa estar visível se movendo.** Antes de animar, confirme que ela não está ocluída por outra geometria nas vistas padrão — foi o que obrigou a colmeia do radiador a virar semitransparente para a ventoinha aparecer.
...
- Exemplo de peça decorativa (sem entrada no catálogo): parafusos de roda e mangueira de respiro do motor (`system` preenchido, `part: ''`).
- Exemplo do pipeline de render completo (ambiente + composer + descarte): bloco logo após a criação das luzes/piso em `CarScene.tsx`, e o cleanup do `useEffect` principal.
