...
- `wear` (opcional, desde `ADR-007`/`TASK-014`): `{ normal, worn, note }` — a comparação entre a peça em bom estado e com uso. Preenchido em 5 peças (pneu, pastilha, disco, correia dentada, mola); a ausência nas outras 29 é o caso normal, não dado faltando. O `note` é obrigatório: é ele que diz o que a comparação **não** permite concluir, e sem ele a comparação vira diagnóstico.
