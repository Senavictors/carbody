...
- Comparação "em bom estado × com uso" (`ADR-007`): `PartDetail` só renderiza o controle quando `part.wear` existe — nunca um controle desabilitado ou um espaço vazio para as 29 peças sem o campo. É um `role="group"` de dois botões com `aria-pressed`, e o texto que muda junto é uma live region `aria-live="polite"`. O estado volta a `'normal'` a cada peça nova, no mesmo `useEffect` que reseta a aba.
