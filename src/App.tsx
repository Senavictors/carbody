...
function App() {
  ...
  const [mechanismMode,setMechanismMode]=useState<'engine'|'gears'>('engine');
  ...
  const openMechanism=(mode:'engine'|'gears')=>{setMechanismMode(mode);navigate('mechanisms');};
      ...
      <main id="main-content" className={`main-content page-${page}`}>
        {page==='explore'&&<>
          ...
          <div className="explorer-layout">
            ...
            <PartDetail part={selectedPart} learned={learned.includes(selectedId)} onLearn={()=>toggleLearned(selectedId)} onMechanism={()=>openMechanism(selectedPart.system==='transmission'?'gears':'engine')}/>
          </div>
        ...
        {page==='mechanisms'&&<><div className="page-heading"><div><h1>O movimento faz sentido<span>.</span></h1><p>Pause, observe e conecte as peças. A mecânica fica mais simples assim.</p></div><button className="subtle-button" onClick={()=>navigate('explore')}><Box size={15}/> Voltar ao carro</button></div><Suspense fallback={<div className="scene-loading" role="status">Preparando os mecanismos…</div>}><MechanismLab key={mechanismMode} initialMode={mechanismMode}/></Suspense><section className="mechanism-connections"><h2>Agora, encontre no carro</h2><div>{parts.filter(part=>['engine','spark-plug','clutch','gearbox'].includes(part.id)).map(part=><button key={part.id} onClick={()=>openPart(part)}><span className={`system-dot system-${part.system}`}/>{part.name}<ArrowUpRight size={16}/></button>)}</div></section><p className="learning-footnote">Os diagramas simplificam um motor de quatro tempos e pares de engrenagens. O projeto real varia conforme o veículo.</p></>}
