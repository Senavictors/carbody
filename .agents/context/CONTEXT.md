...
- **Próximas features** (`ADR-004` a `ADR-007`, 2026-09-13): 4 features planejadas a partir de uma sessão de brainstorm; `TASK-009` em andamento, as demais ainda em `.agents/tasks/backlog/`:
  ...
  - `ADR-006` — glossário de termos técnicos, página própria (sem integrar com a busca `Ctrl K` existente). **`TASK-012` (conteúdo) concluída**: `src/data/glossary.ts` com 19 termos (`GlossaryTerm`: `id`, `term`, `definition`, `relatedParts`, `sourceIds`), em ordem alfabética, sem fonte nova — cada definição reusa as fontes da peça onde o termo aparece. Falta `TASK-013` (página `Page: 'glossary'`, `react-ui`), que consome esse array.
