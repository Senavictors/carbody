...
- **Próximas features** (`ADR-004` a `ADR-007`, 2026-09-13): 4 features planejadas a partir de uma sessão de brainstorm; `TASK-009` em andamento, as demais ainda em `.agents/tasks/backlog/`:
  ...
  - `ADR-007` — comparação visual "desgaste vs. novo". **`TASK-014` (conteúdo) concluída**: campo opcional `wear` (`normal`/`worn`/`note`) na interface `Part`, preenchido nas 5 peças do subconjunto inicial, sem fonte nova — cada afirmação visual foi derivada do que a própria peça já diz em `how`/`signs`/`care`. Falta `TASK-015` (UI + SVG em `PartSketch.tsx`). Lembrar que o escopo de 5 peças foi decisão de planejamento, não escolha explícita do usuário (sinalizado no próprio `ADR-007`), e que a correia dentada é o caso onde a ilustração pode contradizer o texto — ver a seção correspondente em `CONTENT_SOURCES.md` antes de desenhar.
