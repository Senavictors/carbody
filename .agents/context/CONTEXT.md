...
- **Próximas features** (`ADR-004` a `ADR-007`, 2026-09-13): 4 features planejadas a partir de uma sessão de brainstorm; `TASK-009` em andamento, as demais ainda em `.agents/tasks/backlog/`:
  ...
  - `ADR-007` — comparação visual "desgaste vs. novo". **`TASK-014` (conteúdo) concluída**: campo opcional `wear` (`normal`/`worn`/`note`) na interface `Part`, preenchido em 5 peças — pneu, pastilha de freio, disco de freio, vela de ignição e mola —, sem fonte nova: cada afirmação visual foi derivada do que a própria peça já diz em `how`/`signs`/`care`. A correia dentada saiu do subconjunto por decisão do usuário (emenda no `ADR-007`): o catálogo ensina a trocá-la por prazo, não por aparência, e uma ilustração comparativa contradiria isso. Falta `TASK-015` (UI + SVG em `PartSketch.tsx`).
