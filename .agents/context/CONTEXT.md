...
- **Próximas features** (`ADR-004` a `ADR-007`, 2026-09-13): 4 features planejadas a partir de uma sessão de brainstorm; `TASK-009` em andamento, as demais ainda em `.agents/tasks/backlog/`:
  ...
  - `ADR-006` — glossário de termos técnicos como página própria. **Concluído** (`TASK-012` e `TASK-013`): `src/data/glossary.ts` com 19 termos (`GlossaryTerm`), sem fonte nova — cada definição reusa as fontes da peça onde o termo aparece —, e a página `Page: 'glossary'` em `App.tsx`, com busca própria (`termQuery`/`resultTerms`) separada da busca `Ctrl K`, como o ADR decidiu. `Page` tem 5 valores; os chips de cada termo levam à peça via `openPart`.
  - `ADR-007` — comparação visual "desgaste vs. novo". **`TASK-014` (conteúdo) concluída**: campo opcional `wear` (`normal`/`worn`/`note`) na interface `Part`, **Concluído** (`TASK-014` e `TASK-015`): campo opcional `wear` (`normal`/`worn`/`note`) em 5 peças — pneu, pastilha de freio, disco de freio, vela de ignição e mola —, sem fonte nova (cada afirmação visual derivada do que a própria peça já diz em `how`/`signs`/`care`), mais o prop `variant` em `PartSketch` e o controle de comparação em `PartDetail`, que só aparece quando a peça tem o campo. A correia dentada saiu do subconjunto por decisão do usuário (emenda no `ADR-007`): o catálogo ensina a trocá-la por prazo, não por aparência, e uma ilustração comparativa contradiria isso.

- **Saldar a dívida das features** (`ADR-008` e `ADR-009`, 2026-09-13): planejamento das cinco pendências deixadas por `ADR-004` a `ADR-007`. Nenhuma iniciada; todas em `.agents/tasks/backlog/`.
  - `ADR-008` — extrair cada mecanismo do `MechanismLab` (660 linhas) para um módulo próprio em `src/components/mechanisms/`, co-locando dados, diagrama e painel. É a reavaliação que o `ADR-004` pediu. Task: `TASK-017` — a de maior risco das cinco, por tocar os seis mecanismos de uma vez sem suíte de testes.
  - `ADR-009` — validar as referências cruzadas de id (`sourceIds`, `relatedParts`, `mechanismParts`, `PINS`) por script no `npm run build`, formalizando a checagem que foi feita à mão nas `TASK-012`/`TASK-014`. Task: `TASK-018`.
  - Sem ADR, por serem de abordagem única: `TASK-019` (voltar à página de origem ao abrir uma peça — `returnToResults` é booleano e só conhece a biblioteca), `TASK-020` (controle de rotação automática — `autoRotate` está implementado e funcional no `CarScene`, mas `App.tsx` passa `false` fixo), `TASK-021` (animar freios no modelo 3D, o candidato que o `ADR-005` mandou reavaliar) e `TASK-022` (estender `wear` a mais peças, começando pelo filtro de ar).
